...
import datetime
import json
from Emails import envia_email
import dotenv
import os
...
backup_cron = [
    datetime.time(hour=23,  minute=59, second=0, microsecond=0, tzinfo=tz)
]


def enviar_email_backup():
    print(envia_email(EMAILS,
                      f'Auto backup do json',
                      f'Backup do json realizada',
                      'C:\\Users\\Nepuga\\Desktop\\bot xp',
                      ['user_levels.json'])
          )
...
def load_user_levels(filename):
    try:
        ...
        return {}  # Retorna um dicionário vazio40 se o arquivo não existir
...
async def on_thread_create(thread):
    if thread.id not in processed_threads:  # Verifica se o tópico já foi processado
        ...
        # Adiciona o tópico ao conjunto de tópicos processados
...
async def top(ctx):
    dados = load_user_levels(json_filename)
    sorted_users = sorted(
        dados.items(), key=lambda x: x[1]['level'], reverse=True)
